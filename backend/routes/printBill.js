...
router.post('/Print',async (req,res)=>{
    ...
    try {
      ejs.renderFile(path.join(__dirname, '../models/', "template.ejs"), req.body.data , (err, Phtml) => {
        if (err) {
            // res.send(err);
            console.log(err);
        }
            ...
            // console.log(datapd);
            pdf.create(Phtml, options).toFile(`./pdf/Bill${req.body.data.Billno}.pdf`, function (err, data) {
                if (err) {
                    console.log(err);
                } else {
